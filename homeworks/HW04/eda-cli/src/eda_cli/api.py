...
async def quality_flags_from_csv(
    ...
    - Базовые флаги (too_few_rows, too_many_missing и т.д.)
    - Новые флаги из HW03 (has_constant_columns, has_many_zero_values и т.д.)
    """
    start = perf_counter()
    ...
    if df.empty:
        raise HTTPException(
            status_code=400,
            detail="CSV-файл не содержит данных"
        )
    ...
    # Получаем расширенные флаги
    all_flags = compute_extended_quality_flags(df, summary, missing_df, zeros_threshold)
    ...
    response = {
        "dataset_info": {
            ...
            "shape": f"{summary.n_rows}x{summary.n_cols}"
        },
        "quality_scores": {
            "basic_score": all_flags.get("quality_score", 0.0),
            "extended_score": all_flags.get("extended_quality_score", 0.0)
        },
        "flags": {
            # Базовые флаги
            ...
            "max_missing_share": all_flags.get("max_missing_share", 0.0),

            # Новые флаги из HW03
            "has_constant_columns": all_flags.get("has_constant_columns", False),
            "constant_columns_count": all_flags.get("constant_columns_count", 0),
            "constant_columns_list": all_flags.get("constant_columns_list", []),

            "has_many_zero_values": all_flags.get("has_many_zero_values", False),
            "high_zero_columns_count": all_flags.get("high_zero_columns_count", 0),
            "high_zero_columns": all_flags.get("high_zero_columns", {}),

            "has_high_cardinality_categoricals": all_flags.get("has_high_cardinality_categoricals", False),
            "high_cardinality_columns": all_flags.get("high_cardinality_columns", {}),

            "has_suspicious_id_duplicates": all_flags.get("has_suspicious_id_duplicates", False),
            "id_duplicates_info": all_flags.get("id_duplicates_info", {})
        },
        "processing_info": {
            "latency_ms": (perf_counter() - start) * 1000.0,
    ...
    print(
        ...
        f"extended_score={response['quality_scores']['extended_score']:.3f} "
        f"latency_ms={response['processing_info']['latency_ms']:.1f} ms"
    )
